...
phases 1-3 complete: container primitives, OCI images, and networking.
...
networking gives each container its own IP on a bridge network (10.42.0.0/16), with NAT for outbound traffic and port mapping for inbound. containers get a working `/etc/resolv.conf` (copied from host) and `/etc/hosts` automatically.

a security hardening pass addressed all critical and high findings: no `unreachable` in production paths, warning logs for all non-trivial error swallowing, write verification on blob storage, host DNS configuration instead of hardcoded resolvers, and signal handling for clean shutdown.
...
- **phase 4: build engine** — Dockerfile parser, content-hash caching, `yoq build .`
