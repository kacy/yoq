...
pub const Db = struct {
    ...
    pub fn init(options: InitOptions) InitError!Self {
        ...
        // Compute the flags
        const memory_db = switch (options.mode) {
            .Memory => true,
            .File => false,
        };
        ...
        if (options.open_flags.create or (memory_db and options.open_flags.write)) {
...
pub const DynamicStatement = struct {
    ...
    fn bindField(self: *Self, comptime FieldType: type, options: anytype, comptime field_name: []const u8, i: c_int, field: FieldType) !void {
        ...
        switch (FieldType) {
            Text => {
                const result = c.sqlite3_bind_text(self.stmt, column, field.data.ptr, @intCast(field.data.len), null);
                return convertResultToError(result);
            },
            Blob => {
                const result = c.sqlite3_bind_blob(self.stmt, column, field.data.ptr, @intCast(field.data.len), null);
                return convertResultToError(result);
            },
            ZeroBlob => {
