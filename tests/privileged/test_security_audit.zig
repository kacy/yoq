...
test "security audit: API endpoints reject oversized JSON" {
    ...
    // 128KB JSON body — well over the 32KB limit
    var big_body: [128 * 1024]u8 = undefined;
    @memset(&big_body, 'A');
    ...
    big_body[4] = ':';
    big_body[5] = '"';
    big_body[big_body.len - 2] = '"';
    big_body[big_body.len - 1] = '}';

    var resp = http_client.postWithAuth(alloc, addr, port, "/v1/deploy", &big_body, token) catch return;
    defer resp.deinit(alloc);

    // should be rejected — 413 or 400, never 200 or 500
