...
local and cluster execution modes are stored with each job. app and job names beginning with `cluster-` work locally; names do not determine ownership for new records. upgrade migration checks the complete stored app/job identity when classifying older ids. older replicated inserts are classified the same way until their mode is migrated. reloading a cluster job still requires `--server` for control commands.

local owner and control leases use kernel-held file locks. empty or malformed files left by interrupted commands no longer block the next attempt. releasing a lease closes its file descriptor and leaves the lock file in place, so two contenders cannot lock different inodes for the same app.

finish older live apply and training commands before upgrading. the new lease code respects complete pid files owned by an older live process, but old and new lock protocols are not interchangeable. before a downgrade, stop all new apply and training commands and remove the retained `lease` marker files from the data directory's `apply-locks` folder. upgrade all coordinators before submitting new cluster training changes that include execution mode.

local control commands run one at a time. pause, resume, and scale check the stored state before changing it, so a stale command cannot revive a stopped job. scale checks the 1–4,096 rank limit and allocates replacement rank state before pausing. invalid counts and allocation failures leave the running job unchanged. after the old runner exits, yoq saves the new count and pending state together. if another control command is in progress, reload the job status before retrying.
