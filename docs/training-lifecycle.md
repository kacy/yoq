...
cluster rank zero receives an owned rendezvous port. different gangs reserve different ports, including replicated service gangs. the agent keeps the claim until the rank stops; publishing a port failure stops the rank and reports an assignment failure.
