...
| identity | implemented | unique standalone names, separate hostname, rename, explicit duplicate legacy-name errors |
...
saved run configurations are versioned. older files retain their user settings, mount modes, and five-second stop timeout. new containers use a ten-second timeout. existing writable-layer contents cannot be recovered if an older supervisor already deleted them.
