...
`drain_blocked` means the agent must stay online. insufficient capacity is retried as capacity becomes available. a failed replacement, a missing saved placement request, or a local or host volume requires an operator to correct or redeploy the workload. active rollouts finish before their services can move. jobs and training ranks finish in place, or must be stopped explicitly. drain does not copy node-local data. if a blocked worker stops reporting, its assignments remain attached to it; ordinary failover cannot bypass the drain restrictions. inspect assignment status and placement constraints before retrying a blocked drain.
