...
during a cluster rollout, the newest active assignment generation supplies the host's thresholds. generations follow committed placement order, not assignment ids or wall-clock timestamps. older groups can finish draining without restoring their queued notifications. if the newest group retires, the highest remaining generation starts a fresh debounce period. a newer group without thresholds suppresses older thresholds. legacy assignments use generation zero.
...
a process tracks at most 256 active service alert generations. overlapping rollout groups count separately, including empty configurations that suppress older thresholds. inactive entries without an in-flight delivery are reclaimed when that limit is reached. proxy request history holds at most 1,024 service names and 256 samples per service. request sampling uses the global service namespace shared by proxy routes. manifests reject service names already owned by another app, so separate apps cannot share a routed service name.
