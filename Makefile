...
.PHONY: build run test test-operator test-network test-perf-smoke test-golden-path test-integration test-contract test-sim test-gpu test-hardening panic-audit test-runtime-core test-runtime-network test-runtime-cluster test-privileged clean clean-all bpf install fmt loc cache-sqlite release-patch release-minor release-cross
...
test-perf-smoke:
	YOQ_SKIP_SLOW_TESTS=1 $(ZIG) build -Doptimize=ReleaseSafe test-perf-smoke

test-golden-path:
	YOQ_SKIP_SLOW_TESTS=1 $(ZIG) build -Doptimize=ReleaseSafe test-golden-path
