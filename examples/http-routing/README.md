...
the `api` route also rewrites `/api/*` to `/*` before forwarding upstream. route selection prefers the longest matching path, then exact header matches. when the `x-env: canary` header is present, the route selects from the configured weighted backend list and exposes per-route/backend counters in both status and Prometheus output.
