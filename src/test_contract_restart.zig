...
fn expectMissingPath(path: []const u8) !void {
    @import("compat").cwd().access(path, .{}) catch |err| switch (err) {
...
test "contract: s3 route writes durable object bytes to storage" {
    ...
    const raw = try @import("compat").cwd().readFileAlloc(std.testing.allocator, object_path, 1024);
...
test "contract: multipart staging persists on disk until completion and then cleans up" {
    ...
    const meta = try @import("compat").cwd().readFileAlloc(std.testing.allocator, meta_path, 1024);
    ...
    const part1_data = try @import("compat").cwd().readFileAlloc(std.testing.allocator, part1_file, 1024);
