...
fn dnsRegister(entry: *const types.ServiceHealth) void {
    service_registry_bridge.markEndpointHealthy(entry.serviceName(), &entry.container_id, entry.container_ip);
    log.info("health: registered {s} in DNS", .{entry.serviceName()});
}

fn dnsUnregister(entry: *const types.ServiceHealth) void {
    service_registry_bridge.markEndpointUnhealthy(entry.serviceName(), &entry.container_id, entry.container_ip);
