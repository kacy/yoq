...
pub fn runHttpCheck(container_ip: [4]u8, port: u16, path: []const u8, timeout: u32) bool {
    ...
    _ = @import("compat").posix.write(sock, request) catch return false;
...
fn writeGrpcHealthRequest(sock: @import("compat").posix.socket_t, service: []const u8) !void {
...
fn readGrpcHealthResponse(sock: @import("compat").posix.socket_t) bool {
...
fn writeFrame(sock: @import("compat").posix.socket_t, header: http2.FrameHeader, payload: []const u8) !void {
...
fn readFrame(sock: @import("compat").posix.socket_t, payload_storage: []u8) ?ReadFrame {
    var header_buf: [http2.frame_header_len]u8 = undefined;
...
fn writeSettingsAck(sock: @import("compat").posix.socket_t) !void {
    try writeFrame(sock, .{
        .length = 0,
        .frame_type = .settings,
        .flags = 0x01,
...
fn writePingAck(sock: @import("compat").posix.socket_t, opaque_data: []const u8) !void {
    try writeFrame(sock, .{
...
fn tcpConnect(container_ip: [4]u8, port: u16, timeout: u32) ?@import("compat").posix.socket_t {
    const sock = @import("compat").posix.socket(posix.AF.INET, posix.SOCK.STREAM | posix.SOCK.CLOEXEC, 0) catch return null;
    errdefer @import("compat").posix.close(sock);
    ...
    @import("compat").posix.connect(sock, @ptrCast(&addr), @sizeOf(posix.sockaddr.in)) catch return null;
    return sock;
}

fn writeAll(sock: @import("compat").posix.socket_t, bytes: []const u8) !void {
    var written: usize = 0;
    while (written < bytes.len) {
        const chunk = try @import("compat").posix.write(sock, bytes[written..]);
        if (chunk == 0) return error.WriteFailed;
        written += chunk;
    }
}

fn readExact(sock: @import("compat").posix.socket_t, buf: []u8) !void {
...
const BoundTestListener = struct {
    fd: @import("compat").posix.socket_t,
    port: u16,
};
...
fn initTestListenerSocket() !BoundTestListener {
    ...
    while (attempt < 50) : (attempt += 1) {
        ...
        @import("compat").posix.listen(fd, 1) catch {
            if (attempt + 1 == 50) return error.SkipZigTest;
            @import("compat").posix.close(fd);
            @import("compat").sleep(10 * std.time.ns_per_ms);
            continue;
...
fn readGrpcHealthRequestService(sock: @import("compat").posix.socket_t, service_storage: []u8) ?usize {
...
fn writeGrpcHealthTestResponse(sock: @import("compat").posix.socket_t, serving_status: u8, grpc_status: []const u8) !void {
    try writeFrame(sock, .{
        .length = 0,
        .frame_type = .settings,
        .flags = 0,
