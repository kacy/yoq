...
pub const Server = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, port: u16, bind_addr: [4]u8) ServerError!Server {
        // create TCP socket
        const fd = @import("compat").posix.socket(
            posix.AF.INET,
        ...
        const addr = @import("compat").net.Address.initIp4(bind_addr, port);
        @import("compat").posix.bind(fd, &addr.any, addr.getOsSockLen()) catch return ServerError.BindFailed;

        // start listening with a backlog of 128
        @import("compat").posix.listen(fd, 128) catch return ServerError.ListenFailed;
    ...
    fn runBlocking(self: *Server) void {
        while (!orchestrator.shutdown_requested.load(.acquire)) {
            const client_fd = @import("compat").posix.accept(self.listen_fd, null, null, posix.SOCK.CLOEXEC) catch {
                if (orchestrator.shutdown_requested.load(.acquire)) break;
                continue;
            };
