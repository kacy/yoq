...
fn testRealpath(dir: std.Io.Dir, sub_path: []const u8, buf: []u8) ![]const u8 {
...
test "detectProcfs discovers fake GPU directories" {
    ...
    try tmp.dir.createDirPath(std.testing.io, "proc/driver/nvidia/gpus/0000:65:00.0");
    try tmp.dir.createDirPath(std.testing.io, "proc/driver/nvidia/gpus/0000:b3:00.0");
...
test "detectSysfs filters NVIDIA cards from fake drm tree" {
    ...
    try tmp.dir.createDirPath(std.testing.io, "sys/class/drm/card0/device");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "sys/class/drm/card0/device/vendor", .data = "0x10de\n" });
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "sys/class/drm/card0/device/uevent", .data = "PCI_SLOT_NAME=0000:17:00.0\n" });

    try tmp.dir.createDirPath(std.testing.io, "sys/class/drm/card1/device");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "sys/class/drm/card1/device/vendor", .data = "0x8086\n" });
