...
fn testRealpath(dir: platform.Dir, sub_path: []const u8, buf: []u8) ![]const u8 {
...
test "detectProcfs discovers fake GPU directories" {
    ...
    try platform.Dir.from(tmp.dir).makePath("proc/driver/nvidia/gpus/0000:65:00.0");
    try platform.Dir.from(tmp.dir).makePath("proc/driver/nvidia/gpus/0000:b3:00.0");

    var proc_buf: [std.fs.max_path_bytes]u8 = undefined;
    const proc_root = try testRealpath(platform.Dir.from(tmp.dir), "proc/driver/nvidia/gpus", &proc_buf);
...
test "detectSysfs filters NVIDIA cards from fake drm tree" {
    ...
    try platform.Dir.from(tmp.dir).makePath("sys/class/drm/card0/device");
    try platform.Dir.from(tmp.dir).writeFile(.{ .sub_path = "sys/class/drm/card0/device/vendor", .data = "0x10de\n" });
    try platform.Dir.from(tmp.dir).writeFile(.{ .sub_path = "sys/class/drm/card0/device/uevent", .data = "PCI_SLOT_NAME=0000:17:00.0\n" });

    try platform.Dir.from(tmp.dir).makePath("sys/class/drm/card1/device");
    try platform.Dir.from(tmp.dir).writeFile(.{ .sub_path = "sys/class/drm/card1/device/vendor", .data = "0x8086\n" });

    var drm_buf: [std.fs.max_path_bytes]u8 = undefined;
    const drm_root = try testRealpath(platform.Dir.from(tmp.dir), "sys/class/drm", &drm_buf);
