...
pub fn writePrometheus(writer: anytype, metrics: [max_gpus]?GpuMetrics, count: u8) !void {
    ...
    for (0..@min(count, max_gpus)) |i| {
        if (metrics[i]) |m| {
            try writer.print("yoq_gpu_temperature_celsius{{gpu=\"{d}\"}} {d}\n", .{ i, m.temperature_c });
        }
    }
    ...
    for (0..@min(count, max_gpus)) |i| {
        if (metrics[i]) |m| {
            try writer.print("yoq_gpu_utilization_ratio{{gpu=\"{d}\"}} {d:.2}\n", .{ i, @as(f64, @floatFromInt(m.utilization_gpu)) / 100.0 });
        }
    }
    ...
    for (0..@min(count, max_gpus)) |i| {
        if (metrics[i]) |m| {
            try writer.print("yoq_gpu_memory_used_bytes{{gpu=\"{d}\"}} {d}\n", .{ i, m.memory_used_mb * 1024 * 1024 });
        }
    }
    ...
    for (0..@min(count, max_gpus)) |i| {
        if (metrics[i]) |m| {
            try writer.print("yoq_gpu_memory_total_bytes{{gpu=\"{d}\"}} {d}\n", .{ i, m.memory_total_mb * 1024 * 1024 });
        }
    }
    ...
    for (0..@min(count, max_gpus)) |i| {
        if (metrics[i]) |m| {
            try writer.print("yoq_gpu_power_watts{{gpu=\"{d}\"}} {d}\n", .{ i, m.power_watts });
        }
    }
    ...
    for (0..@min(count, max_gpus)) |i| {
        if (metrics[i]) |m| {
            try writer.print("yoq_gpu_ecc_errors_total{{gpu=\"{d}\",type=\"single_bit\"}} {d}\n", .{ i, m.ecc_errors_single });
            try writer.print("yoq_gpu_ecc_errors_total{{gpu=\"{d}\",type=\"double_bit\"}} {d}\n", .{ i, m.ecc_errors_double });
        }
    }
}
...
test "writeMetricsJson empty" {
    ...
    try writeMetricsJson(&fbs, metrics, 0);
    try std.testing.expectEqualStrings("\"gpu_metrics\":[]", fbs.buffered());
}
...
test "writeMetricsJson with one GPU" {
    ...
    try writeMetricsJson(&fbs, metrics, 1);
    const json = fbs.buffered();
...
test "writePrometheus empty" {
    ...
    var fbs: std.Io.Writer = .fixed(&buf);
    const metrics: [max_gpus]?GpuMetrics = .{null} ** max_gpus;
    try writePrometheus(&fbs, metrics, 0);
...
test "writePrometheus with one GPU" {
    ...
    try writePrometheus(&fbs, metrics, 1);
    const output = fbs.buffered();
...
test "writePrometheus with multiple GPUs" {
    ...
    try writePrometheus(&fbs, metrics, 2);
    const output = fbs.buffered();
