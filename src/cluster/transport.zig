...
const invalid_socket: linux_platform.posix.socket_t = -1;
...
pub const Transport = struct {
    ...
    pub fn init(alloc: std.mem.Allocator, port: u16) !Transport {
        const fd = try linux_platform.posix.socket(posix.AF.INET, posix.SOCK.STREAM | posix.SOCK.NONBLOCK, 0);
        ...
        try linux_platform.posix.listen(fd, 16);
    ...
    pub fn deinit(self: *Transport) void {
        self.deinitUdp();
        if (self.listen_fd != invalid_socket) linux_platform.posix.close(self.listen_fd);
        self.peers.deinit();
    }

    pub fn addPeer(self: *Transport, id: NodeId, addr: [4]u8, port: u16) !void {
        try self.peers.put(id, .{
            .addr = linux_platform.net.Address.initIp4(addr, port),
        });
    }
    ...
    fn resolvePeerId(self: *const Transport, addr: linux_platform.net.Address) ?NodeId {
...
test "verifyAuthenticatedBody rejects mismatched sender id" {
    ...
    try std.testing.expectError(
        ...
        verifyAuthenticatedBody(&body, key, linux_platform.net.Address.initIp4(.{ 10, 0, 0, 2 }, 9000), &peers),
    );
}
...
test "verifyAuthenticatedBody rejects sender from wrong ip" {
    ...
    var peers = std.AutoHashMap(NodeId, PeerAddr).init(alloc);
    defer peers.deinit();
    try peers.put(2, .{ .addr = linux_platform.net.Address.initIp4(.{ 10, 0, 0, 2 }, 9700) });

    try std.testing.expectError(
        TransportError.AuthenticationFailed,
        verifyAuthenticatedBody(&body, key, linux_platform.net.Address.initIp4(.{ 10, 0, 0, 99 }, 40000), &peers),
    );
}
...
test "resolvePeerId matches configured peer" {
    ...
    try std.testing.expectEqual(
        @as(?NodeId, 2),
        transport.resolvePeerId(linux_platform.net.Address.initIp4(.{ 10, 0, 0, 2 }, 9700)),
    );
    try std.testing.expect(
        transport.resolvePeerId(linux_platform.net.Address.initIp4(.{ 10, 0, 0, 3 }, 9700)) == null,
    );
}
...
fn requireUdpGossipTestHost() !void {
    ...
    const fd = linux_platform.posix.socket(posix.AF.INET, posix.SOCK.DGRAM | posix.SOCK.CLOEXEC, 0) catch
...
test "udp gossip: send and receive with HMAC" {
    ...
    try linux_platform.posix.getsockname(receiver.udp_fd.?, @ptrCast(&recv_addr), &recv_len);
