...
pub fn readSnapshotMeta(path: []const u8) SnapshotError!SnapshotMeta {
    const file = @import("compat").cwd().openFile(path, .{}) catch return SnapshotError.IoError;
    defer file.close();
...
pub fn takeSnapshot(self: anytype, dest_path: []const u8, meta: SnapshotMeta) SnapshotError!void {
    ...
    defer @import("compat").cwd().deleteFile(tmp.path) catch {};
    tmp.file.close();
    ...
    const tmp_data = @import("compat").cwd().readFileAlloc(std.heap.page_allocator, tmp.path, @intCast(max_snapshot_size)) catch {
        return SnapshotError.IoError;
    };
    defer std.heap.page_allocator.free(tmp_data);

    const file = @import("compat").cwd().createFile(dest_path, .{ .truncate = true, .mode = 0o600 }) catch return SnapshotError.IoError;
    defer file.close();
...
pub fn restoreFromSnapshot(self: anytype, src_path: []const u8) SnapshotError!SnapshotMeta {
    const data = @import("compat").cwd().readFileAlloc(std.heap.page_allocator, src_path, @intCast(max_snapshot_size)) catch {
        return SnapshotError.IoError;
    };
...
fn createUniqueTempFile(buf: []u8, prefix: []const u8, suffix: []const u8) SnapshotError!struct {
    ...
    while (attempts < 16) : (attempts += 1) {
        const slice = std.fmt.bufPrint(buf, "{s}.{x}{s}", .{ prefix, @import("compat").randomInt(u64), suffix }) catch {
            return SnapshotError.IoError;
        };
        if (slice.len >= buf.len) return SnapshotError.IoError;
        buf[slice.len] = 0;
        const path: [:0]const u8 = buf[0..slice.len :0];
        const file = @import("compat").cwd().createFile(path, .{ .exclusive = true, .mode = 0o600 }) catch |err| switch (err) {
