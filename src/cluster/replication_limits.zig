...
// older leaders admitted larger commands before checking transport capacity.
// carry one such entry intact, while reserving the existing envelope bytes.
pub const max_inherited_frame_bytes: usize = @import("transport/common.zig").max_receive_size -
    @import("transport/auth_support.zig").authenticated_header_size + 4;
