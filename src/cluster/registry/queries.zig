...
pub fn getAssignments(alloc: Allocator, db: *sqlite.Db, agent_id: []const u8) ![]Assignment {
    return queryAssignmentRows(
        alloc,
        db,
        "SELECT id, agent_id, image, command, status, status_reason, cpu_limit, memory_limit_mb, app_name, workload_kind, workload_name, health_check_json, gang_rank, gang_world_size, gang_master_addr, gang_master_port, generation FROM assignments WHERE agent_id = ?;",
        .{agent_id},
    );
}
...
pub fn getOrphanedAssignments(alloc: Allocator, db: *sqlite.Db) ![]Assignment {
    return queryAssignmentRows(
        alloc,
        db,
        "SELECT id, agent_id, image, command, status, status_reason, cpu_limit, memory_limit_mb, app_name, workload_kind, workload_name, health_check_json, gang_rank, gang_world_size, gang_master_addr, gang_master_port, generation FROM assignments WHERE agent_id = '' AND status = 'pending';",
        .{},
    );
}
...
pub fn listAssignmentsForWorkload(
    ...
    return queryAssignmentRows(
        alloc,
        db,
        "SELECT id, agent_id, image, command, status, status_reason, cpu_limit, memory_limit_mb, app_name, workload_kind, workload_name, health_check_json, gang_rank, gang_world_size, gang_master_addr, gang_master_port, generation FROM assignments WHERE app_name = ? AND workload_kind = ? AND workload_name = ? ORDER BY created_at, id;",
