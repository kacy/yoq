...
pub fn save(alloc: std.mem.Allocator, service: []const u8, producer: []const u8, boot: []const u8, samples: []const Sample, now_ns: u64) !void {
    if (samples.len > samples_per_writer) return error.TooManySamples;
    const json = try std.json.Stringify.valueAlloc(alloc, samples, .{});
    ...
    try lease.db.exec(
        ...
        .{},
        .{ service, producer, boot, @as(i64, @intCast(now_ns)), json },
    );
    try lease.db.exec("DELETE FROM proxy_observations WHERE service = ? AND rowid NOT IN (SELECT rowid FROM proxy_observations WHERE service = ? ORDER BY updated_ns DESC, rowid DESC LIMIT 8);", .{}, .{ service, service });
