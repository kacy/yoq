...
fn loadOne(alloc: Allocator, comptime query: []const u8, args: anytype) StoreError!ImageRecord {
    const db = try common.getDb();
    const row = (db.oneAlloc(ImageRow, alloc, query, .{}, args) catch return StoreError.ReadFailed) orelse
...
pub fn removeImage(id: []const u8) StoreError!void {
    ...
    db.exec("DELETE FROM images WHERE id = ?;", .{}, .{id}) catch return StoreError.WriteFailed;
}
