...
test "process identity resolves numeric and named users and explicit groups" {
...
test "process identity kernel retains denied rootless default groups but rejects explicit identity" {
...
test "process identity resolves image accounts and changes actual child credentials" {
    if (linux.geteuid() != 0) return error.SkipZigTest;
    var tmp = std.testing.tmpDir(.{});
    defer tmp.cleanup();
    try tmp.dir.createDirPath(std.testing.io, "etc");
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "etc/passwd", .data = "app:x:12345:23456::/:/bin/sh\n" });
    try tmp.dir.writeFile(std.testing.io, .{ .sub_path = "etc/group", .data = "staff:x:34567:\n" });
    var root: [4096]u8 = undefined;
    const length = try tmp.dir.realPathFile(std.testing.io, ".", &root);
    root[length] = 0;
    const pid = linux.fork();
    if (linux.errno(pid) != .SUCCESS) return error.ForkFailed;
    if (pid == 0) {
        if (linux.errno(linux.chroot(root[0..length :0])) != .SUCCESS) linux.exit_group(10);
        platform.posix.chdir("/") catch linux.exit_group(11);
        const account = resolve("app:staff") catch linux.exit_group(12);
        apply(account, false) catch linux.exit_group(13);
        if (linux.getuid() != 12345 or linux.geteuid() != 12345) linux.exit_group(14);
        if (linux.getgid() != 34567 or linux.getegid() != 34567) linux.exit_group(15);
        if (linux.getgroups(0, null) != 0) linux.exit_group(16);
        linux.exit_group(0);
    }
    const result = try @import("process.zig").wait(@intCast(pid), false);
    try std.testing.expectEqual(@import("process.zig").ExitStatus{ .exited = 0 }, result.status);
}
