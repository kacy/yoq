...
pub fn pivotRoot(new_root: []const u8) FilesystemError!void {
    ...
    @import("compat").posix.chdir(new_root) catch return FilesystemError.PivotFailed;
...
pub fn bindMount(target_root: []const u8, source: []const u8, target: []const u8, read_only: bool) FilesystemError!void {
    ...
    defer @import("compat").posix.close(validation_fd);
    ...
    @import("compat").cwd().makePath(target_buf[0..target_pos]) catch return FilesystemError.MkdirFailed;
