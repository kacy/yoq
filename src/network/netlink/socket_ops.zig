...
pub fn openSocket() common.NetlinkError!posix.fd_t {
    ...
    return @import("compat").posix.socket(
        linux.AF.NETLINK,
...
pub fn resolveFamily(fd: posix.fd_t, name: []const u8) common.NetlinkError!u16 {
    ...
    const sent = @import("compat").posix.send(fd, msg, 0) catch return common.NetlinkError.SendFailed;
    if (sent != msg.len) return common.NetlinkError.SendFailed;

    var recv_buf: [common.buf_size]u8 align(4) = undefined;
    const recv_len = @import("compat").posix.recv(fd, &recv_buf, 0) catch return common.NetlinkError.RecvFailed;
