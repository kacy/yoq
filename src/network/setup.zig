...
test "writeNetworkFiles sets resolv.conf to bridge gateway" {
    ...
    const rootfs_path = @import("compat").Dir.from(tmp_dir.dir).realpath(".", &path_buf) catch return;
    ...
    const content = @import("compat").cwd().readFileAlloc(alloc, resolv_path, 4096) catch return;
...
test "writeNetworkFiles sets etc/hosts with hostname" {
    ...
    const content = @import("compat").cwd().readFileAlloc(alloc, hosts_path, 4096) catch return;
    defer alloc.free(content);
