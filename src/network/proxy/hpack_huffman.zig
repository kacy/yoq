...
pub const Error = error{InvalidHuffman} || std.mem.Allocator.Error;

const max_huffman_code_len: u8 = 30;

pub fn decodeString(alloc: std.mem.Allocator, encoded: []const u8) Error![]u8 {
    ...
    while (bit_pos < total_bits) {
        ...
        while (probe_pos < total_bits and code_len < max_huffman_code_len) : (probe_pos += 1) {
            ...
            code_len += 1;
            if (findSymbol(code, code_len)) |symbol| {
...
fn findSymbol(code: u32, code_len: u8) ?u8 {
    for (codes, 0..) |candidate_code, symbol_index| {
        if (code_lens[symbol_index] == code_len and candidate_code == code) {
