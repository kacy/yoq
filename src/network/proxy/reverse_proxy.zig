...
pub const ReverseProxy = struct {
    ...
    pub fn deinit(self: *ReverseProxy) void {
        while (self.active_mirror_requests.load(.acquire) != 0) {
            @import("compat").sleep(std.time.ns_per_ms);
        }
    }
...
fn peerIpFromSocket(fd: @import("compat").posix.socket_t) ?[4]u8 {
...
fn readResponse(alloc: std.mem.Allocator, fd: @import("compat").posix.socket_t, max_bytes: usize) ![]u8 {
...
fn readRequestBytes(fd: @import("compat").posix.socket_t, buf: []u8) ReadRequestError![]const u8 {
    var total: usize = 0;
    while (total < buf.len) {
        ...
        for (self.actions, 0..) |action, index| {
            ...
            switch (action) {
                ...
                .stream_respond => |resp| {
                    ...
                    @import("compat").sleep(@as(u64, resp.delay_ms) * std.time.ns_per_ms);
...
fn captureRequestBytes(fd: @import("compat").posix.socket_t, buf: []u8) usize {
    var total: usize = 0;
    while (total < buf.len) {
        const bytes_read = posix.read(fd, buf[total..]) catch break;
        if (bytes_read == 0) break;
...
fn readSocketBytes(fd: @import("compat").posix.socket_t, buf: []u8) usize {
    var total: usize = 0;
    while (total < buf.len) {
        const bytes_read = posix.read(fd, buf[total..]) catch break;
        if (bytes_read == 0) break;
...
test "handleConnection proxies a client socket request" {
    ...
    const ConnectionHarness = struct {
        fn serve(proxy_ptr: *const ReverseProxy, listen_fd: posix.fd_t) void {
            const client_fd = @import("compat").posix.accept(listen_fd, null, null, posix.SOCK.CLOEXEC) catch return;
    ...
    defer @import("compat").posix.close(client_fd);
    const server_addr = @import("compat").net.Address.initIp4(.{ 127, 0, 0, 1 }, listener.port);
    try @import("compat").posix.connect(client_fd, &server_addr.any, server_addr.getOsSockLen());
...
test "handleConnection relays HTTP/2 client data frames upstream" {
    ...
    try socket_helpers.writeAll(client_fd, request);
    @import("compat").sleep(25 * std.time.ns_per_ms);
...
test "handleConnection rejects looped request after listener restart" {
    ...
    var client_fd = try @import("compat").posix.socket(posix.AF.INET, posix.SOCK.STREAM | posix.SOCK.CLOEXEC, 0);
    const server_addr = @import("compat").net.Address.initIp4(.{ 127, 0, 0, 1 }, listener.port);
    try @import("compat").posix.connect(client_fd, &server_addr.any, server_addr.getOsSockLen());
    try socket_helpers.writeAll(client_fd, "GET / HTTP/1.1\r\nHost: api.internal\r\nX-Yoq-Proxy: 1\r\n\r\n");
    ...
    client_fd = try @import("compat").posix.socket(posix.AF.INET, posix.SOCK.STREAM | posix.SOCK.CLOEXEC, 0);
    const restarted_addr = @import("compat").net.Address.initIp4(.{ 127, 0, 0, 1 }, listener.port);
    try @import("compat").posix.connect(client_fd, &restarted_addr.any, restarted_addr.getOsSockLen());
    try socket_helpers.writeAll(client_fd, "GET / HTTP/1.1\r\nHost: api.internal\r\nX-Yoq-Proxy: 1\r\n\r\n");
